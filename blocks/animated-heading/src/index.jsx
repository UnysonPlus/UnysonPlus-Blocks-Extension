/**
 * Animated Heading — the block editor half.
 *
		...
		return (
			<div { ...blockProps }>
				...
				{ /*
				  ...
				  * working copy.
				  *
				  * The markup the server returns IS the finished element, so nothing is
				  * replayed over it. What that means for THIS block is that the words do
				  * not rotate in the canvas: the preview shows the first word, held.
				  *
				  * That is the better preview, not a limitation worked around. A headline
				  * that retyped itself every few seconds would move while you were reading
				  * the sidebar, and every option change re-renders the preview — so the
				  * animation would restart from the beginning each time, which reads as the
				  * editor stuttering rather than as a preview of anything. Visitors see the
				  * full rotation.
				  */ }
				<Disabled>
					<div
