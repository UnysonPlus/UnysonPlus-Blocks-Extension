/**
 * Special Heading — the block editor half.
 *
...
registerBlockType( metadata.name, {
	edit( { attributes, setAttributes } ) {
		...
		/*
		 * Overline, title and subtitle all default to an empty string, so a freshly
		 * inserted block renders literally nothing — an invisible block the user
		 * cannot see to click. A Placeholder until at least one of the three has
		 * text is the difference between "not filled in yet" and "broken".
		 */
		const hasContent = !! (
			( upOptions.overline || '' ).trim() ||
			( upOptions.title || '' ).trim() ||
			( upOptions.subtitle || '' ).trim()
		);
		return (
			<div { ...blockProps }>
				<Inspector
