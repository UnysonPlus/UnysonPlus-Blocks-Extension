/**
 * Icon Box — the block editor half.
 *
...
registerBlockType( metadata.name, {
	edit( { attributes, setAttributes } ) {
		...
		/*
		 * No Placeholder branch: an Icon Box with no title or text still renders
		 * its icon and box styling, so a fresh insert shows a real (if empty)
		 * card rather than nothing. The user fills it in from the sidebar.
		 */
		return (
			<div { ...blockProps }>
				<Inspector
				...
				{ /*
				  ...
				  * copy of it. Icon Box renders entirely server-side and ships no
				  * front-end JS, so there is no runtime to replay — the markup the
				  * server returns IS the finished element. `Disabled` additionally
				  * stops the whole-box link from navigating away when the block is
