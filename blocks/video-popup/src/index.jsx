/**
 * Video Popup — the block editor half.
 *
...
registerBlockType( metadata.name, {
	edit( { attributes, setAttributes } ) {
		...
		/*
		 * No Placeholder branch, deliberately. The shortcode already renders its
		 * own guidance ("Add a poster image and a video URL.") when neither a
		 * poster nor a source is set AND it is running in an admin/AJAX context —
		 * which is exactly what ServerSideRender is. Duplicating that check here
		 * would give two empty states free to disagree with each other.
		 */
		return (
			<div { ...blockProps }>
				<Inspector
				...
				{ /*
				  ...
				  * copy of it. Video Popup's only script is a delegated click handler
				  * that opens the lightbox — nothing about layout — so the poster and
				  * play button render correctly with no runtime replayed here, and a
				  * click in the canvas should select the block rather than open a video.
				  *
